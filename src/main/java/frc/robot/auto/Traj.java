...
public class Traj implements IFollowable{
    ...
    public void generate(State prevState){
        if(this.type == TrajType.MOVING){
            PathPlannerPath pppath = isChoreo ? PathPlannerPath.fromChoreoTrajectory(filename) : PathPlannerPath.fromPathFile(filename);
            double rotvel = prevState.holonomicAngularVelocityRps.orElse(prevState.headingAngularVelocityRps);
            ChassisSpeeds cs = new ChassisSpeeds(prevState.velocityMps * Math.cos(Math.PI * 0.25),
